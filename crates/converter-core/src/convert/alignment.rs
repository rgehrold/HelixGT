...
pub fn convert_alignment(
    ...
    match (input_format, output_format) {
        ...
        (FileFormat::Cram, FileFormat::Cram) => copy_cram(input_path, output_path),
        ...
        (FileFormat::Cram, FileFormat::Bam) => cram_to_bam(input_path, output_path),
        (FileFormat::Sam, FileFormat::Cram) | (FileFormat::Bam, FileFormat::Cram) => {
            anyhow::bail!(
                "writing CRAM requires a reference FASTA (not yet supported).\n\
                 Try converting to SAM or BAM first."
            )
        }
...
pub fn copy_cram(input_path: &Path, output_path: &Path) -> Result<u64> {
    let file = File::open(input_path)
    ...
    for (index, result) in reader.records(&header).enumerate() {
        let record = result.with_context(|| format!("invalid CRAM record #{}", index + 1))?;
        writer
            .write_record(&header, record)
            .with_context(|| format!("failed to write CRAM record #{}", index + 1))?;
        count += 1;
    }
    Ok(count)
}
...
fn cram_to_sam(input_path: &Path, output_path: &Path, options: &ConvertOptions) -> Result<u64> {
    ...
    for (index, result) in reader.records(&header).enumerate() {
        ...
        let alignment = record
            ...
            .with_context(|| format!("failed to decode CRAM record #{}", index + 1))?;
        writer
...
fn cram_to_bam(input_path: &Path, output_path: &Path) -> Result<u64> {
    let file = File::open(input_path)
        .with_context(|| format!("failed to open CRAM '{}'", input_path.display()))?;
    let mut reader = cram::io::Reader::new(file);
    let header = reader.read_header().context("failed to read CRAM header")?;
...
fn cram_to_fasta(input_path: &Path, output_path: &Path, options: &ConvertOptions) -> Result<u64> {
    ...
    for (index, result) in reader.records(&header).enumerate() {
        ...
        let alignment = record
            .try_into_alignment_record(&header)
            .with_context(|| format!("failed to decode CRAM record #{}", index + 1))?;
        if write_alignment_as_fasta(&mut writer, &alignment, index)? {
            count += 1;
        }
    }
...
fn cram_to_fastq(input_path: &Path, output_path: &Path, options: &ConvertOptions) -> Result<u64> {
    ...
    for (index, result) in reader.records(&header).enumerate() {
        ...
        let alignment = record
            .try_into_alignment_record(&header)
            .with_context(|| format!("failed to decode CRAM record #{}", index + 1))?;
        if write_alignment_as_fastq(&mut writer, &alignment, index)? {
            count += 1;
        }
    }
    finish_gzip_writer(writer.into_inner())?;
